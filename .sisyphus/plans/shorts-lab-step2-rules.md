...
### Definition of Done
- [x] 2단계규칙 탭에서 3종 프롬프트를 저장/복구할 수 있다.
- [x] 백업은 5개까지만 유지되고, 복구 시 UI가 즉시 반영된다.
- [x] 2단계 생성 요청에 새 프롬프트가 반영된다(네트워크 payload 확인).
...
- [x] 1. 2단계 규칙 저장/백업 구조 정의

  **What to do**:
  - 2단계 규칙의 데이터 구조(3개 프롬프트 필드) 정의.
   - 저장/로드/백업/복구 흐름 설계 (백업 5개 제한).
  ...
  **Acceptance Criteria**:
  - [x] 규칙 데이터 구조가 3개 텍스트 필드로 정의됨.
   - [x] 백업이 5개까지만 유지되도록 정책이 명시됨.
  - [x] 저장/복구 실패 시 에러 메시지 경로가 정의됨.

- [x] 2. 장르 관리 모달에 "2단계규칙" 탭 UI 추가
  ...
  **Acceptance Criteria**:
  - [x] 2단계규칙 탭이 보이고 3개 텍스트 영역이 표시됨.
  - [x] 저장/백업/복구 버튼이 동작하고 토스트/에러 처리 확인됨.
   - [x] 백업 리스트가 5개를 넘지 않음.

- [x] 3. 2단계 생성 로직에 새 규칙 적용
  ...
  **Acceptance Criteria**:
  - [x] 저장된 규칙이 2단계 생성 요청 payload에 포함됨.
  - [x] 수정 전/후 프롬프트가 네트워크 payload에서 확인됨.
...
### Final Checklist
- [x] 2단계규칙 탭에서 3종 프롬프트 저장/복구 가능
- [x] 백업 5개 제한 동작 확인
- [x] 2단계 생성 요청에 수정된 프롬프트 반영 확인
