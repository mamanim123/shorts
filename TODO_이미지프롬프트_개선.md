...
### 6. 캐릭터별 의상 고유성 문제 해결 (2026-02-03 완료) 🎉
**위치**:
- `services/shortsLabStep2PromptRulesDefaults.ts:122, 174-175` (AI 규칙)
- `youtube-shorts-script-generator.tsx:33, 670-730, 732-745, 1978-1988, 2662-2672` (AI 대본 생성)
- `services/labPromptBuilder.ts:1557-1602, 1629-1675` (의상 선택 로직)

- [x] **Phase 1**: 문제 분석 - 잘못된 색상 변경 방식 발견
- [x] **Phase 2**: 에이전트 분석 - AI 대본 생성 전체 플로우 파악
- [x] **Phase 3**: `pickFemaleOutfit` / `pickMaleOutfit` import 추가
- [x] **Phase 4**: `validateAndFixOutfitUniqueness` 완전 재작성
  - ❌ 색상 변경 방식 삭제 (가짜 의상 생성)
  - ✅ 의상리스트에서 랜덤 재선택 (실제 의상 사용)
- [x] **Phase 5**: `postProcessScripts` 파라미터에 genre, topic 추가
- [x] **Phase 6**: 호출 지점 수정 (generateScript, handleManualImport)
- [x] **Phase 7**: ShortsLabPanel.tsx 잘못된 코드 제거
- [x] **Phase 8**: TypeScript 컴파일 검증 및 문서화 완료

**해결된 문제**:
- ✅ 여러 캐릭터가 동일한 의상 착용 문제 (WomanA = WomanB outfit ❌ → 각자 고유 의상 ✅)
- ✅ **의상리스트에서 랜덤으로 재선택** (가짜 의상 생성 ❌ → 실제 의상 사용 ✅)
- ✅ 겨울 자동 변환 지원 (pickFemaleOutfit 내부에서 처리)
- ✅ 장르별 의상 필터링 (SEXY, GOLF 등)

**작동 방식**:
1. **1차 중복 방지** (buildLabScriptPrompt) - pickFemaleOutfit/pickMaleOutfit + excludeOutfits
2. **2차 중복 방지** (validateAndFixOutfitUniqueness) - AI가 무시했을 경우 의상리스트에서 재선택
3. **2단계 생성** - 이미 1차 중복 방지가 완벽하므로 후처리 불필요

**핵심 개선**:
```typescript
// 이전 (잘못된 방식)
newOutfit = outfit.replace(/White/g, 'Cream'); // ❌ 가짜 의상

// 현재 (올바른 방식)
newOutfit = pickFemaleOutfit(genre, topic, [사용된의상들]); // ✅ 실제 의상
```

**상세 문서**: `plans/outfit_fix_proper_완료보고.md`
