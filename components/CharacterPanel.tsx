...
const CharacterPanel: React.FC<CharacterPanelProps> = ({
  ...
  const [isDraggingTurnaround, setIsDraggingTurnaround] = useState(false);
  ...
  const [activeSlotId, setActiveSlotId] = useState(selectedSlot);
  ...
  const [collapsedCharacterSections, setCollapsedCharacterSections] = useState({
    coreBody: true,
    hair: true,
    body: true
  });
  ...
  const effectiveSelectedSlot = activeSlotId || selectedSlot || slotOptions[0]?.id || 'woman-a';
  ...
  const isLocalAiStudioCharacterId = useCallback((id?: string | null) => (
    typeof id === 'string' && id.startsWith('aistudio-')
  ), []);
  ...
  const handleDeleteCharacter = useCallback(async (id: string) => {
    ...
    if (isLocalAiStudioCharacterId(id)) {
      try { await deleteCharacterFolder(id); } catch (e) { /* 서버 폴더가 없을 수 있음 */ }
      const updated = characters.filter((char) => char.id !== id);
      setCharacters(updated);
      await saveCharactersToBE(updated);
      onCharacterDeleted?.(target);
      if (selectedCharacterId === id) {
        setSelectedCharacterId(null);
        onCharacterSelect?.(null, effectiveSelectedSlot);
      }
      ...
      try {
        ...
        if (!res.ok || !payload?.success) {
          throw new Error(payload?.error || '폴더 삭제 실패');
	        ...
	        onCharactersSaved?.(refreshed as any);
	        onCharacterDeleted?.(target);
	        if (selectedCharacterId === id) {
	          setSelectedCharacterId(null);
	          onCharacterSelect?.(null, effectiveSelectedSlot);
    ...
    if (target.sourceType === 'ai-studio') {
      ...
      onCharacterSelect?.(null, effectiveSelectedSlot);
	    ...
	    showToast('캐릭터가 삭제되었습니다.', 'success');
	  }, [characters, effectiveSelectedSlot, isLocalAiStudioCharacterId, onCharacterDeleted, onCharacterSelect, onCharactersSaved, saveCharactersToBE, selectedCharacterId]);
  ...
  const handleTurnaroundSourceFile = useCallback((file: File | null | undefined) => {
    if (!file) return;
...
Output one single image for this viewpoint only.`;
      ...
      const saveRes = await fetch('http://localhost:3002/api/save-character-turnaround', {
        ...
        headers: { 'Content-Type': 'application/json' },
            ...
            outfitFit: newCharacter.outfitFit
          }
        })
      });
      ...
      if (!saveRes.ok || !savePayload?.success) {
        throw new Error(savePayload?.error || '캐릭터 폴더 저장 실패');
	      ...
	      setCharacters(refreshed as any);
	      onCharactersSaved?.(refreshed as any);
	      const savedCharacter = savePayload.folderName
	        ? refreshed.find((char) => char.id === savePayload.folderName)
	        : refreshed.find((char) => char.name === newCharacter.name.trim());
		      if (savedCharacter) {
		        setSelectedCharacterId(savedCharacter.id);
		        if (!selectedCharacterId) {
		          onCharacterAdded?.(savedCharacter);
		        }
		        onCharacterSelect?.(savedCharacter, effectiveSelectedSlot);
		      }
		      setTurnaroundSource(null);
		      setTurnaroundProgress('');
		      showToast(`${newCharacter.name} 3면도가 ${selectedCharacterId ? '수정되어 저장' : '생성되어 저장'}되었습니다.`, 'success');
		      setActiveTab('select');
    } catch (err) {
	  ...
	  }, [turnaroundSource, newCharacter, selectedCharacterId, effectiveSelectedSlot, onCharacterAdded, onCharacterSelect, onCharactersSaved]);
  ...
  const handleSaveCharacter = useCallback(() => {
    ...
    if (selectedCharacterId) {
      ...
      const updated = characters.map((char) => (
        char.id === selectedCharacterId ? updatedCharacter : char
	      ...
	      onCharacterSelect?.(updatedCharacter, effectiveSelectedSlot);
    ...
    showToast(`${character.name} 캐릭터가 저장되었습니다.`, 'success');
	  }, [newCharacter, characters, generateStyleFromBody, selectedCharacterId, bodyTuning, effectiveSelectedSlot, onCharacterAdded, onCharacterSelect, saveCharactersToBE]);
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3 mb-2">
                <div>
                  <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">저장된 캐릭터</div>
                  <div className="text-[10px] text-slate-600 mt-1">캐릭터를 클릭하면 현재 대본 슬롯에 연결하고, 관리 화면에서 수정합니다.</div>
                </div>
                <button
                  type="button"
                  onClick={handleStartNewCharacter}
                  className="shrink-0 px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold transition-all flex items-center gap-1.5 shadow-lg shadow-emerald-950/20"
                >
              ...
              {characters.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 border-2 border-dashed border-slate-800 rounded-xl text-slate-600">
                  ...
                  <button
                    type="button"
                    onClick={handleStartNewCharacter}
                    className="mt-3 px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold transition-all flex items-center gap-1.5"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    캐릭터 추가
                  </button>
                    ...
                    <div
                      ...
                      onClick={() => {
                        ...
                        setNewCharacter({
                          ...
                          outfitFit: char.outfitFit || ''
                        });
                        setBodyTuning(char.bodyTuning ? { ...DEFAULT_BODY_TUNING, ...char.bodyTuning } : { ...DEFAULT_BODY_TUNING });
                        setActiveTab('manage');
	                        const selectedSlotMeta = slotOptions.find((slot) => slot.id === effectiveSelectedSlot);
	                        const targetSlot = selectedSlotMeta?.gender === char.gender
	                          ? effectiveSelectedSlot
	                          : (slotOptions.find((slot) => slot.gender === char.gender)?.id || effectiveSelectedSlot);
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2">
	                            <div className="text-sm font-bold text-slate-200 group-hover:text-purple-300 transition-colors">{char.name || '이름 없음'}</div>
                            {char.sourceType === 'ai-studio' && (
	                          ...
	                          <div className="mt-2 rounded-lg border border-slate-800 bg-slate-950/45 px-2 py-1.5">
	                            <div className="text-[9px] font-bold text-slate-500">내부 ID</div>
	                            <div className="text-[10px] text-slate-400 break-all leading-snug" title={char.id}>{char.id}</div>
	                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
        ...
        {activeTab === 'manage' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/50 space-y-3">
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">이름</label>
                <p className="text-[10px] text-slate-500 mb-1.5">화면과 대본에 표시되는 한글 이름입니다. 내부 ID는 그대로 유지됩니다.</p>
                <input
                  type="text"
                  placeholder="예: 지영, 철수"
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="예: 도시적인 커리어우먼, 차분한 관찰자"
                  value={newCharacter.identity}
                  onChange={e => setNewCharacter(prev => ({ ...prev, identity: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder-slate-600 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Outfit Fit</label>
                <p className="text-[10px] text-slate-500 mb-1.5">의상 이름보다 핏과 실루엣을 적습니다. 의상규칙과 이미지 프롬프트에 함께 연결됩니다.</p>
                <input
                  type="text"
                  placeholder="예: 슬림핏 정장, 오버사이즈 니트"
                  value={newCharacter.outfitFit}
                  onChange={e => setNewCharacter(prev => ({ ...prev, outfitFit: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder-slate-600 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-bold text-slate-300 mb-1">연령</label>
                  <select
                    value={newCharacter.age}
                    onChange={e => setNewCharacter(prev => ({ ...prev, age: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-sm text-slate-200 outline-none focus:ring-2 focus:ring-purple-500/50"
                  >
                    {['20대', '30대', '40대', '50대', '60대'].map(age => <option key={age} value={age}>{age}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-slate-300 mb-1">성별</label>
                  <select
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Face</label>
                <p className="text-[10px] text-slate-500 mb-1.5">얼굴형, 인상, 표정, 대표 이미지에서 유지할 얼굴 특징입니다.</p>
                <textarea
                  placeholder="예: soft oval face, gentle eyes, calm expression"
                  value={newCharacter.face}
                  onChange={e => setNewCharacter(prev => ({ ...prev, face: e.target.value }))}
                  rows={2}
                  className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder-slate-600 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all resize-none"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Hair</label>
                <p className="text-[10px] text-slate-500 mb-1.5">머리 길이, 색, 스타일처럼 장면마다 유지할 헤어 문구입니다.</p>
                <textarea
                  placeholder="예: elegant high ponytail, long brown wavy hair"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => setNewCharacter(prev => ({ ...prev, style: e.target.value }))}
                  rows={2}
                  className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder-slate-600 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all resize-none"
                />
              </div>
		              <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-3">
		                <div className="flex items-center justify-between gap-2 mb-2">
		                  <div>
		                    <div className="text-[10px] font-bold text-slate-300">캐릭터 프롬프트 미리보기</div>
		                    <div className="text-[10px] text-slate-500 mt-0.5">위 필드가 합쳐져 이미지 생성과 의상규칙에 전달되는 최종 캐릭터 문구입니다.</div>
		                  </div>
		                  <button
	                    type="button"
	                    onClick={() => handleCopyPrompt([
	                      newCharacter.identity,
	                      newCharacter.face,
	                      newCharacter.hair,
	                      newCharacter.body,
	                      newCharacter.style,
	                      newCharacter.outfitFit ? `Outfit fit: ${newCharacter.outfitFit}` : ''
	                    ].filter(Boolean).join(', '), 'character-preview')}
	                    className="px-2 py-1 rounded-md bg-slate-800 hover:bg-slate-700 text-[10px] font-bold text-slate-300 transition-colors"
	                  >
	                    복사
	                  </button>
	                </div>
	                <textarea
	                  ...
	                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-300 resize-none outline-none"
	                  placeholder="입력한 캐릭터 필드가 여기에 합쳐져 표시됩니다."
	                />
	              </div>
	            </div>
            ...
            <div className="bg-slate-800/40 border border-purple-700/40 rounded-xl p-3 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold text-purple-200">3면도 생성 (정면/45도/뒷모습)</span>
	              ...
	              <label
	                className={`flex flex-col items-center justify-center py-5 border-2 border-dashed rounded-xl cursor-pointer transition-all ${isDraggingTurnaround
	                  ? 'border-purple-400 bg-purple-500/10 scale-[1.01]'
	                  : 'border-slate-700 hover:border-purple-500/60 hover:bg-purple-500/5'
	                  }`}
	                onDragOver={(e) => {
	                  e.preventDefault();
	                  e.stopPropagation();
	                  setIsDraggingTurnaround(true);
	                }}
	                onDragLeave={(e) => {
	                  e.preventDefault();
	                  e.stopPropagation();
	                  if (e.currentTarget.contains(e.relatedTarget as Node)) return;
	                  setIsDraggingTurnaround(false);
	                }}
	                onDrop={(e) => {
	                  e.preventDefault();
	                  e.stopPropagation();
	                  setIsDraggingTurnaround(false);
	                  handleTurnaroundSourceFile(e.dataTransfer.files?.[0]);
	                }}
	              >
	                <input
	                  type="file"
	                  ...
	                  className="hidden"
	                  onChange={(e) => {
	                    handleTurnaroundSourceFile(e.target.files?.[0]);
	                    e.target.value = '';
	                  }}
	                />
	                {turnaroundSource ? (
	                  ...
	                  <>
	                    <Upload size={20} className={`${isDraggingTurnaround ? 'text-purple-300 animate-bounce' : 'text-slate-500'} mb-1`} />
	                    <span className={`text-[10px] ${isDraggingTurnaround ? 'text-purple-300' : 'text-slate-500'}`}>
	                      {isDraggingTurnaround ? '여기에 이미지를 놓으세요' : '기준 인물 사진 드래그 또는 클릭'}
	                    </span>
	                  </>
	                )}
	              </label>
              ...
              >
                {isGeneratingTurnaround ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
	                {isGeneratingTurnaround ? '3면도 생성 중...' : (selectedCharacterId ? '3면도 수정 후 저장' : '3면도 생성 후 저장')}
              </button>
            ...
            <div className="space-y-2 pt-1">
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">보조 설정</div>

              <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 overflow-hidden">
                <button
                  type="button"
                  onClick={() => setCollapsedCharacterSections(prev => ({ ...prev, coreBody: !prev.coreBody }))}
                  className="w-full px-3 py-3 flex items-center justify-between gap-3 text-left hover:bg-slate-800/60 transition-colors"
                >
                  <div>
                    <div className="text-[11px] font-bold text-purple-300">핵심 체형 조절</div>
                    <div className="text-[10px] text-slate-500 mt-0.5">가슴, 다리, 엉덩이 등 주요 체형값을 세부 조절합니다.</div>
                  </div>
                  <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${collapsedCharacterSections.coreBody ? '' : 'rotate-180'}`} />
                </button>
                {!collapsedCharacterSections.coreBody && (
              ...
              <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 overflow-hidden">
                <button
                  type="button"
                  onClick={() => setCollapsedCharacterSections(prev => ({ ...prev, hair: !prev.hair }))}
                  className="w-full px-3 py-3 flex items-center justify-between gap-3 text-left hover:bg-slate-800/60 transition-colors"
                >
                  <div>
                    <div className="text-[11px] font-bold text-purple-300">헤어스타일</div>
                    <div className="text-[10px] text-slate-500 mt-0.5">프리셋을 선택하거나 헤어 문구를 직접 보정합니다.</div>
                  </div>
                  <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${collapsedCharacterSections.hair ? '' : 'rotate-180'}`} />
                </button>
                {!collapsedCharacterSections.hair && (
                  <div className="px-3 pb-3 space-y-3 border-t border-slate-700/50">
              ...
              <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 overflow-hidden">
                <button
                  type="button"
                  onClick={() => setCollapsedCharacterSections(prev => ({ ...prev, body: !prev.body }))}
                  className="w-full px-3 py-3 flex items-center justify-between gap-3 text-left hover:bg-slate-800/60 transition-colors"
                >
                  <div>
                    <div className="text-[11px] font-bold text-purple-300">체형</div>
                    <div className="text-[10px] text-slate-500 mt-0.5">체형 사진 분석, 프리셋, 체형/포즈 문구를 보정합니다.</div>
                  </div>
                  <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${collapsedCharacterSections.body ? '' : 'rotate-180'}`} />
                </button>
                {!collapsedCharacterSections.body && (
                  <div className="px-3 pb-3 space-y-3 border-t border-slate-700/50">
                    <label
                      className={`mt-3 flex flex-col items-center justify-center py-6 border-2 border-dashed rounded-xl cursor-pointer transition-all group ${isDraggingBody
