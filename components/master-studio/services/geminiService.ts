...
export const initGeminiService = async () => {
    ...
    try {
        const response = await fetch('http://localhost:3002/api/auth/gemini-key');
        if (response.ok) {
...
const withRetry = async <T>(apiCall: () => Promise<T>, usageContext?: string): Promise<T> => {
    ...
    const usageKeyId = getActiveApiKeyId();

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const result = await apiCall();
            if (usageContext) {
                recordUsage(usageContext, result, usageKeyId);
            }
            return result;
        } catch (error) {
            ...
            // Check for retryable error codes (503, 429)
            if (errorMessage.includes('"code":503') || errorMessage.includes('"status":"unavailable"') || errorMessage.includes('resource_exhausted')) {
                if (attempt < maxRetries) {
                    const delay = Math.pow(2, attempt) * 1000; // 2s, 4s
                    console.log(`API call failed (Attempt ${attempt}/${maxRetries}). Retrying in ${delay / 1000}s... Error: ${lastError.message}`);
    ...
    // If we're here, all attempts failed. Throw the last captured error.
    throw lastError;
};
...
export const generateImage = async (prompt: string, config: any, safetySettings?: any, imageParts?: { inlineData: { data: string, mimeType: string } }[]): Promise<GenerateContentResponse> => {
    ...
    return withRetry(() => ai.models.generateContent({
        model: config.model || 'gemini-1.5-flash', // Fallback to standard Gemini model
        contents: { parts },
        config: {
            ...config,
