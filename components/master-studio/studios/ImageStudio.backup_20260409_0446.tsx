...
type StoredImageState = { file: File; url: string; id: string };

type TurnaroundViewKey = 'front' | 'angle45' | 'back';

type TurnaroundPreview = {
    key: TurnaroundViewKey;
    label: string;
    imageId: string;
    url: string;
};

type PendingTurnaroundSet = {
    sourceImageId: string;
    sourcePrompt: string;
    previews: Record<TurnaroundViewKey, TurnaroundPreview>;
    approved: boolean;
};
...
const extractImagePartData = (response: GenerateContentResponse | any): { data: string; mimeType: string } | null => {
    if (response && 'generatedImages' in response) {
        if (response.generatedImages && response.generatedImages.length > 0) {
            const generatedImage = response.generatedImages[0];
            if (generatedImage?.image?.imageBytes) {
                return { data: generatedImage.image.imageBytes, mimeType: 'image/png' };
            }
        }
    } else if (response) {
        ...
        if (inlineData?.data) {
            return { data: inlineData.data, mimeType: 'image/png' };
        }
    }
...
const TURNAROUND_PROMPTS: Record<TurnaroundViewKey, { label: string; instruction: string }> = {
    front: {
        label: '정면',
        instruction: 'Generate a full-body front view turnaround shot of the exact same person from the reference image.'
    },
    angle45: {
        label: '45도',
        instruction: 'Generate a full-body 45-degree three-quarter turnaround shot of the exact same person from the reference image.'
    },
    back: {
        label: '뒷모습',
        instruction: 'Generate a full-body back view turnaround shot of the exact same person from the reference image.'
    }
};
...
const ImageStudio: React.FC = () => {
    ...
    const [originalImage, setOriginalImage] = useState<StoredImageState | null>(null);
    const [characterReference, setCharacterReference] = useState<StoredImageState | null>(null);
    const [backgroundReference, setBackgroundReference] = useState<StoredImageState | null>(null);
    const [templateReference, setTemplateReference] = useState<StoredImageState | null>(null);
    ...
    const createStoredImageStateFromPart = useCallback(async (imagePartData: { data: string; mimeType: string }, fileName: string): Promise<StoredImageState> => {
        ...
        const file = new File([imageBlob], fileName, { type: imageBlob.type || imagePartData.mimeType || 'image/png' });
        const url = URL.createObjectURL(imageBlob);
        return { file, url, id: imageId };
    }, []);

    const getTurnaroundSourceImage = useCallback((): StoredImageState | null => {
        return characterReference || originalImage || null;
    }, [characterReference, originalImage]);

    const handleGenerateTurnaround = useCallback(async () => {
        const sourceImage = getTurnaroundSourceImage();
        if (!sourceImage) {
            setNotification({ message: '먼저 캐릭터 기준 이미지를 업로드해주세요.', type: 'error' });
            return;
        }
        ...
        try {
            ...
            const referencePart = { inlineData: { data: sourceBase64, mimeType: sourceImage.file.type } };
            const effectiveModel = imageModel.toLowerCase().includes('imagen') ? 'gemini-2.5-flash-image' : imageModel;
            const previews = {} as Record<TurnaroundViewKey, TurnaroundPreview>;
...
Output one single image for this viewpoint only.`;
                ...
                const stored = await createStoredImageStateFromPart(imagePartData, `turnaround-${key}.png`);
                previews[key] = {
                    key,
                    label: meta.label,
                    imageId: stored.id,
            ...
            setGeneratedImage({ url: previews.front.url, id: previews.front.imageId });
            setNotification({ message: '캐릭터 3면도 시안이 생성되었습니다. 검토 후 승인해주세요.', type: 'success' });
        } catch (err) {
            console.error('Turnaround generation failed:', err);
            const message = err instanceof Error ? err.message : '3면도 생성 실패';
            setError(message);
            setNotification({ message, type: 'error' });
        } finally {
            setIsGeneratingTurnaround(false);
        }
    }, [createStoredImageStateFromPart, getTurnaroundSourceImage, imageModel, noGuard, prompt]);

    const handleApproveTurnaround = useCallback(() => {
        if (!pendingTurnaroundSet) return;
        setPendingTurnaroundSet({ ...pendingTurnaroundSet, approved: true });
        setNotification({ message: '3면도 시안이 승인되었습니다. 이제 저장할 수 있습니다.', type: 'success' });
    }, [pendingTurnaroundSet]);

    const handleSaveApprovedTurnaround = useCallback(() => {
        if (!pendingTurnaroundSet?.approved) {
            setNotification({ message: '먼저 3면도 결과를 승인해주세요.', type: 'error' });
            return;
        }
    ...
    const loadCharacter = async (character: CharacterCollection) => {
        ...
        const representativeImageId = character.turnaroundImageIds?.front || character.generatedImageId;

        if (representativeImageId) {
            try {
                const blob = await getBlob(representativeImageId);
                if (blob) {
        ...
        const turnaroundTag = character.turnaroundImageIds
            ? 'front, 45-degree, and back turnaround references are locked'
            : 'single reference image is locked';
        const characterTag = `[Character: ${character.description}; ${turnaroundTag}]`;
        setPrompt(prev => {
            ...
            <div className="w-[45%] flex flex-col p-4 space-y-3 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col space-y-3 overflow-y-auto pr-2 -mr-2">
                    ...
                    <div className="p-4 bg-gray-900/50 rounded-lg border border-white/10">
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                ...
                                                                {character.turnaroundImageIds && (
                                                                    <p className="text-[10px] text-cyan-300 mt-1">3면도 저장됨</p>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </button>
                        ...
                        <div className="mb-3 bg-gray-800/60 border border-cyan-500/20 rounded-lg p-3 space-y-3">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    <h3 className="text-sm font-bold text-cyan-300">캐릭터 3면도 생성</h3>
                                    <p className="text-xs text-gray-400 mt-1">
                                        업로드한 기준 이미지로 정면, 45도, 뒷모습 시안을 만들고 마마님 승인 후 직접 저장합니다.
                                    </p>
                                </div>
                                <button
                                    onClick={handleGenerateTurnaround}
                                    disabled={isGeneratingTurnaround || !getTurnaroundSourceImage()}
                                    className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${isGeneratingTurnaround || !getTurnaroundSourceImage()
                                        ? 'bg-gray-700 text-gray-400 cursor-not-allowed'
                                        : 'bg-cyan-600 hover:bg-cyan-500 text-white'}`}
                                >
                                    {isGeneratingTurnaround ? '생성 중...' : '3면도 생성'}
                                </button>
                            </div>
                            ...
                            {pendingTurnaroundSet && (
                                <div className="space-y-3">
                                    <div className="grid grid-cols-3 gap-2">
                                        {(['front', 'angle45', 'back'] as TurnaroundViewKey[]).map((key) => {
                                            const preview = pendingTurnaroundSet.previews[key];
                                            return (
                                                <button
                                                    key={key}
                                                    onClick={() => setGeneratedImage({ url: preview.url, id: preview.imageId })}
                                                    className="bg-gray-900/70 border border-gray-700 hover:border-cyan-400 rounded-lg p-2 text-left transition-colors"
                                                >
                                                    <img src={preview.url} alt={preview.label} className="w-full h-28 object-cover rounded mb-2" />
                                                    <p className="text-xs font-semibold text-gray-200">{preview.label}</p>
                                                </button>
                                            );
                                        })}
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            onClick={handleApproveTurnaround}
                                            disabled={pendingTurnaroundSet.approved}
                                            className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${pendingTurnaroundSet.approved
                                                ? 'bg-emerald-900/50 text-emerald-200 border border-emerald-500/30'
                                                : 'bg-emerald-600 hover:bg-emerald-500 text-white'}`}
                                        >
                                            {pendingTurnaroundSet.approved ? '승인 완료' : '결과 승인'}
                                        </button>
                                        <button
                                            onClick={handleSaveApprovedTurnaround}
                                            disabled={!pendingTurnaroundSet.approved}
                                            className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${pendingTurnaroundSet.approved
