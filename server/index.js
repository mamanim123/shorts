...
const CHARACTER_TURNAROUND_DIR = path.join(GENERATED_DIR, 'characters');
...
if (!fs.existsSync(CHARACTER_TURNAROUND_DIR)) fs.mkdirSync(CHARACTER_TURNAROUND_DIR, { recursive: true });
...
app.post('/api/translate-to-english', async (req, res) => {
    try {
        ...
        const normalizedType = ['outfit', 'face', 'hair', 'body', 'scene'].includes(type) ? type : 'face';
...
English prompt:`
                : normalizedType === 'scene'
                    ? `You are a cinematic prompt translator for AI image generation. Translate this Korean scene prompt into a natural, detailed English prompt optimized for image models.

RULES:
- Preserve the original scene meaning exactly
- Keep character identity, clothing, action, facial expression, camera, background, props, and atmosphere
- Expand short Korean phrases into natural English image-prompt wording when needed
- Do NOT add unrelated style changes or remove details
- Output ONLY the English prompt, nothing else
...
// ---------------------------------------------------------
// 영문 → 한글 번역 API (씬 프롬프트용)
// ---------------------------------------------------------
app.post('/api/translate-to-korean', async (req, res) => {
    try {
        const { text, type } = req.body;
        if (!text) return res.status(400).json({ success: false, error: '번역할 텍스트가 없습니다.' });

        const normalizedType = ['scene'].includes(type) ? type : 'scene';
        console.log(`[Translate] English → Korean (${normalizedType}): ${text.substring(0, 50)}...`);

        const translatePrompt = `You are a cinematic prompt translator for AI image generation. Translate this English scene prompt into natural Korean that is easy for a creator to read and edit.

RULES:
- Preserve the original meaning exactly
- Keep character identity, clothing, action, facial expression, camera, background, props, and atmosphere
- Keep prompt details useful for later re-translation into English
- Use fluent Korean prompt wording, not awkward literal translation
- Output ONLY the Korean prompt, nothing else
...
Korean prompt:`;
        ...
        console.error('한글 번역 API 에러:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
app.post('/api/save-character-turnaround', async (req, res) => {
    try {
        const { characterName, sourceImageData, turnaroundImages, metadata, characterId, targetFolderName } = req.body || {};
        if (!characterName || typeof characterName !== 'string') {
            return res.status(400).json({ success: false, error: '캐릭터 이름이 없습니다.' });
        }
        if (!turnaroundImages || typeof turnaroundImages !== 'object') {
            return res.status(400).json({ success: false, error: '3면도 이미지 데이터가 없습니다.' });
        }

        const requestedId = String(targetFolderName || characterId || '').trim();
        const canUseRequestedId = requestedId
        ...
        if (fs.existsSync(metaPath)) {
            try { previousMeta = JSON.parse(fs.readFileSync(metaPath, 'utf8')) || {}; } catch (e) { previousMeta = {}; }
        }
        fs.mkdirSync(targetDir, { recursive: true });

        const saveBase64Png = (filename, imageData) => {
        ...
        res.json({
            success: true,
            updated: !!canUseRequestedId,
            folderName,
            folderPath: path.join('generated_scripts', 'characters', folderName),
            folderUrl: `/generated_scripts/characters/${folderName}`,
            files: {
                source: sourceImageData ? `/generated_scripts/characters/${folderName}/source.png` : null,
        ...
        console.error('캐릭터 3면도 폴더 저장 실패:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
// 캐릭터 관리 API
// ---------------------------------------------------------
// [1단계 통합] 캐릭터 정체성의 단일 진실 = generated_scripts/characters 폴더.
// characters.json 에 의존하지 않고 항상 폴더를 스캔한다. 실제 이미지가 있는 캐릭터만 반환.
    ...
    try {
        ...
        if (fs.existsSync(charDir)) {
            ...
            for (const d of folders) {
                ...
                const folderPath = path.join(charDir, folderName);
                let meta = {};
                try { meta = JSON.parse(fs.readFileSync(path.join(folderPath, 'character.json'), 'utf8')); } catch (e) {}
                const files = meta.files || {};
                const has = (f) => f && fs.existsSync(path.join(folderPath, f));
                const url = (f) => has(f) ? base + '/' + encodeURIComponent(folderName) + '/' + f : undefined;

                // 실제 정면 이미지가 없으면 껍데기로 보고 건너뛴다 (미리보기 불가 캐릭터 제거)
                const frontFile = files.front || 'front.png';
                if (!has(frontFile)) continue;
                ...
                characters.push({
                    ...
                    age: md.age || meta.age || '40s',
                    gender: md.gender || meta.gender || 'female',
                    face: md.face || meta.face || '',
                    hair: md.hair || meta.hair || '',
                    body: md.body || meta.body || '',
                    style: md.style || meta.style || '',
                    identity: md.identity || meta.identity || meta.description || '',
                    outfitFit: md.outfitFit || meta.outfitFit || '',
                    bodyTuning: md.bodyTuning || meta.bodyTuning,
                    identitySpec: md.identitySpec || meta.identitySpec,
                    referencePreference: md.referencePreference || meta.referencePreference,
                    wardrobeProfile: md.wardrobeProfile || meta.wardrobeProfile,
                    createdAt: meta.createdAt || new Date().toISOString(),
                    sourceType: 'ai-studio',
                    generatedImageId: url(frontFile),
                    turnaroundImageIds: {
                        front: url(files.front || 'front.png'),
                        angle45: url(files.angle45 || 'angle45.png'),
                        back: url(files.back || 'back.png')
                    },
                    threeviewUrl: url(files.threeview || 'threeview.png'),
                    sourceUrl: url(files.source || 'source.png')
                });
            }
        }
        // 최신순 정렬 (폴더명이 타임스탬프로 시작하므로 역순)
        characters.sort((a, b) => String(b.id).localeCompare(String(a.id)));
        res.json({ characters });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
                ...
                if (fRawParsedData) {
                    ...
                    if (Array.isArray(scenesSource)) {
                        fParsedScenes = scenesSource.map((scene, idx) => ({
                            ...
                            characterIds: scene.characterIds || scene.characterSlot || [],
                            characterSlot: scene.characterSlot || '',
