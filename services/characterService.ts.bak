...
export const saveCharacters = async (characters: CharacterItem[]): Promise<boolean> => {
  ...
  const aiStudioCollectionMap = new Map(
    existingAiStudioCollections.map((collection) => [collection.id, collection])
  );

  aiStudioCharacters.forEach((character) => {
    const collectionId = character.id.replace(/^aistudio-/, '');
    const existing = aiStudioCollectionMap.get(collectionId);
    if (!existing) return;
    aiStudioCollectionMap.set(collectionId, {
      ...existing,
      name: character.name,
      description: character.style || character.face || existing.description,
      age: character.age,
  ...
  await setAppStorageValue('characterCollection', Array.from(aiStudioCollectionMap.values()));
