...
const API_ENDPOINT = 'http://localhost:3002/api/engine-config';
...
const fetchEngineConfig = async () => {
    try {
        ...
        console.error('[enginePromptStore] Failed to load engine config:', e);
...
export const ensureEngineConfigLoaded = async () => {
    // Force reload every time to retrieve latest config from server
    // if (configLoaded) return { prompts: promptsCache, options: optionsCache };
    return await fetchEngineConfig();
};

export const loadEnginePrompts = (): Record<EngineKey, string> => {
    return promptsCache;
};

export const getEnginePrompt = (engine: EngineKey): string => {
    return promptsCache[engine] ?? '';
};

const formatAvailableEngines = () => {
    const keys = Object.keys(promptsCache || {}).filter(Boolean);
...
export const resolveEnginePrompt = (engine: EngineKey): string => {
    ...
    if (!prompt || !prompt.trim()) {
        ...
        // [FIX] 에러를 던지는 대신 기본 V3 엔진으로 폴백하여 생성이 중단되지 않게 함
...
export const saveEnginePrompt = async (engine: EngineKey, prompt: string) => {
    ...
    try {
        ...
        console.error('[enginePromptStore] Failed to save engine prompt:', e);
    }
};
...
export const resetEnginePrompt = (engine: EngineKey) => {
    ...
    saveEnginePrompt(engine, defaultPrompt);
};

export const getDefaultEnginePrompt = (engine: EngineKey): string => {
    return defaultPrompts[engine];
};

export const loadEngineOptions = (): EngineOption[] => {
    return optionsCache;
};
...
export const saveEngineOptions = async (options: EngineOption[]) => {
    ...
    try {
        ...
        console.error('[enginePromptStore] Failed to save engine options:', e);
    }
};
