...
export const LAB_GENRE_GUIDELINES: Record<string, {
  ...
  supportingCharacterPhrasePatterns?: string[];  // ⭐ 조연 대사 패턴
  bodyReactions: string[];
  forbiddenPatterns: string[];
  goodTwistExamples: string[];
  supportingCharacterTwistPatterns?: string[];   // ⭐ 조연 활용 반전 패턴
...
→ ⭐ 조연(캐디, 스태프, 점원 등)이 등장하면 나중에 반전에 활용!

[BUILD-UP] 😳 뭔가 이상함 감지
→ "근데 사람들이 자꾸 쳐다보는 거야"
→ 신체반응 필수: 찜찜함, 불안감

[CLIMAX] 😱 진실 발견 순간 - ⭐ 조연이 반전을 드러내면 더 재밌음!
→ 거울, 사진, 누군가의 지적으로 발견
→ "그때 (누가) '언니/형 저기...' 하는 거야"
→ ⭐ 특히 조연(캐디, 스태프, 우연히 만난 지인)이 지적하면 창피함 극대화!
→ ⭐ "알고 보니 캐디가 와이프 친구/동생이었어" 같은 연결고리 활용!

[TWIST] 🤦 민망한 진실 공개
→ "알고보니 (창피한 상황)이었던 거야"
→ SETUP의 복선과 연결!
→ ⭐ 조연이 반전의 핵심 역할을 하면 스토리가 더 탄탄해짐!
...
`,
    ...
    // ⭐ 조연이 반전을 드러내는 대사 패턴 (주제에 맞게 새롭게 창작!)
    supportingCharacterPhrasePatterns: [
      ...
      '조연이 과거 만남을 언급하는 대사 ("저번에 OO랑 오셨던 분!")',
    ],
    ...
    // ⭐ 조연 활용 반전 패턴 (이 원칙으로 주제에 맞게 새롭게 창작!)
    supportingCharacterTwistPatterns: [
      '조연(캐디, 스태프, 점원 등)이 화자의 가족/지인과 뜻밖의 연결고리가 있어서 들킴',
      '조연이 민망한 상황/실수를 우연히 목격하고 지적함',
      '조연의 한마디가 반전의 트리거가 됨 (예: "어? 이거 왜 이래요?")',
      '조연이 나중에 가족/지인에게 전해서 들킴',
      '조연이 화자를 알아보면서 창피함 극대화',
    ],
...
`,
    ...
    // ⭐ 조연 활용 설렘 패턴 (이 원칙으로 주제에 맞게 새롭게 창작!)
    supportingCharacterTwistPatterns: [
...
`,
    ...
    // ⭐ 조연(목격자) 활용 반전 패턴 (이 원칙으로 주제에 맞게 새롭게 창작!)
    supportingCharacterTwistPatterns: [
