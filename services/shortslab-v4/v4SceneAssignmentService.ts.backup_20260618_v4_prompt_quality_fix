  ...
  "scenes": [
    {
      ...
      "imagePrompt": "complete English image generation prompt for this scene"
    }
  ]
}`;
};
...
export const parseV4SceneAssignment = (rawText: string, allowedSlotIds: string[]): V4ScenePlan[] => {
  ...
  return source.map((scene: any, index: number): V4ScenePlan => {
    ...
    return {
      ...
      imagePrompt: String(scene.imagePrompt || scene.rawPrompt || scene.prompt || scene.longPrompt || '').trim(),
      rawPrompt: String(scene.rawPrompt || scene.imagePrompt || scene.prompt || '').trim()
    };
