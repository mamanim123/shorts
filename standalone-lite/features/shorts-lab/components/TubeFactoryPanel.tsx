...
// 기존 로직 서비스
import { initGeminiService, generateImageWithImagen, generateMultiSpeakerAudio, getApiKey } from './master-studio/services/geminiService';
import { generateBenchmarkStorylinePackage, generateStory } from '../services/geminiService';
...
const TTS_ENGINES = [
  { id: 'multilingual-v2', label: 'Multilingual v2', description: '29개 언어 지원, 저연산 미리듣기', available: false, credit: '500자당 1P' },
  { id: 'elevenlabs', label: 'ElevenLabs Flash v2.5', description: '빠르고 자연스러운 프리셋 엔진', available: false, credit: '1000자당 1P' },
  { id: 'typecast', label: 'Typecast', description: '보이스 캐릭터형 합성 준비중', available: false, credit: '500자당 1.5P' },
  { id: 'gemini', label: 'Gemini TTS', description: 'Google AI 음성 합성, 현재 연결됨', available: true, credit: 'API 키 사용' },
];
...
const GEMINI_VOICE_LIBRARY = [
  { id: 'Zephyr', name: '제피르', subtitle: '밝음, 경쾌함', language: 'ja', gender: '중성', popular: false },
  { id: 'Puck', name: '도현', subtitle: '업비트, 예능감', language: 'ko', gender: '남성', popular: true },
  { id: 'Charon', name: '브라이언', subtitle: '설명형, 정보 전달', language: 'ko', gender: '남성', popular: true },
  { id: 'Kore', name: '슬기', subtitle: '단단함, 한국어 추천', language: 'ko', gender: '여성', popular: true },
  { id: 'Fenrir', name: '하이룩', subtitle: '신남, 텐션 높음', language: 'ko', gender: '남성', popular: false },
  { id: 'Leda', name: '레다', subtitle: '젊음, 부드러움', language: 'en', gender: '여성', popular: true },
  { id: 'Orus', name: '오루스', subtitle: '단단함, 선명함', language: 'en', gender: '남성', popular: false },
  { id: 'Aoede', name: '세나', subtitle: '산뜻함, 차분함', language: 'ko', gender: '여성', popular: true },
  { id: 'Callirrhoe', name: '칼리로에', subtitle: '편안함, 이지리스닝', language: 'en', gender: '여성', popular: false },
  { id: 'Autonoe', name: '오토노에', subtitle: '밝음, 또렷함', language: 'en', gender: '여성', popular: false },
  { id: 'Enceladus', name: '엔셀라두스', subtitle: '브리시, 속삭임', language: 'en', gender: '남성', popular: false },
  { id: 'Iapetus', name: '이아페투스', subtitle: '클리어, 교정된 톤', language: 'en', gender: '남성', popular: false },
  { id: 'Umbriel', name: '엄브리엘', subtitle: '여유로움, 부드러움', language: 'en', gender: '중성', popular: false },
  { id: 'Algieba', name: '알지에바', subtitle: '스무스, 안정감', language: 'en', gender: '남성', popular: false },
  { id: 'Despina', name: '데스피나', subtitle: '스무스, 라디오형', language: 'en', gender: '여성', popular: false },
  { id: 'Erinome', name: '에리노메', subtitle: '깨끗함, 정돈된 톤', language: 'en', gender: '여성', popular: false },
  { id: 'Algenib', name: '알게닙', subtitle: '허스키, 거친 질감', language: 'en', gender: '남성', popular: false },
  { id: 'Rasalgethi', name: '라살게티', subtitle: '정보형, 차분한 해설', language: 'en', gender: '남성', popular: false },
  { id: 'Laomedeia', name: '라오메데이아', subtitle: '활기참, 예능형', language: 'en', gender: '여성', popular: false },
  { id: 'Achernar', name: '아케르나르', subtitle: '소프트, 조용한 톤', language: 'en', gender: '여성', popular: false },
  { id: 'Alnilam', name: '알닐람', subtitle: '단단함, 앵커형', language: 'en', gender: '남성', popular: false },
  { id: 'Schedar', name: '셰다르', subtitle: '균형감, 무난함', language: 'en', gender: '중성', popular: false },
  { id: 'Gacrux', name: '가크룩스', subtitle: '성숙함, 중후함', language: 'ko', gender: '남성', popular: false },
  { id: 'Pulcherrima', name: '풀케리마', subtitle: '전진감, 강조형', language: 'en', gender: '여성', popular: false },
  { id: 'Achird', name: '아키르드', subtitle: '친근함, 캐주얼', language: 'en', gender: '여성', popular: false },
  { id: 'Zubenelgenubi', name: '주베넬게누비', subtitle: '캐주얼, 가벼움', language: 'en', gender: '중성', popular: false },
  { id: 'Vindemiatrix', name: '빈데미아트릭스', subtitle: '부드러움, 온화함', language: 'en', gender: '여성', popular: false },
  { id: 'Sadachbia', name: '사다크비아', subtitle: '생동감, 밝음', language: 'en', gender: '남성', popular: false },
  { id: 'Sadaltager', name: '사달타게르', subtitle: '지식형, 안정감', language: 'en', gender: '남성', popular: false },
  { id: 'Sulafat', name: '술라팟', subtitle: '따뜻함, 포근함', language: 'en', gender: '여성', popular: false },
] as const;

type Step = 'status-board' | 'project-list' | 'style' | 'script' | 'tts' | 'character' | 'media' | 'image-effect' | 'edit' | 'subtitle' | 'seo' | 'export' | 'tubeflow' | 'fusion';
...
const TubeFactoryPanel: React.FC = () => {
  ...
  const [ttsLanguage, setTtsLanguage] = useState<AppLanguage>('ko');
  ...
  const [voiceFilter, setVoiceFilter] = useState<FavoriteFilterId>('popular');
  ...
  const handleGenerateVariation = async (sceneId: number) => {
    ...
    try {
      ...
      const imageUrl = await generateImageWithImagen(finalPrompt, '', { aspectRatio: '9:16' });
  ...
  // 3. 캐릭터 일관성 데이터 (TubeFactory 원본 벤치마킹 스타일)
...
${selectedStoryDraft || selectedStory.content}${benchmarkContext}`;
  ...
  // ============================================
  // [로직] 2. 실제 이미지 생성 연동 (Imagen 3 + 캐릭터 퓨전)
  ...
  const handleGenerateImage = async (sceneId: number) => {
    ...
    try {
      ...
      // 캐릭터가 할당되어 있다면, 캐릭터 설정을 프롬프트에 주입 + 참조 이미지 추출
      if (scene.assignedCharacter) {
        const char = characters.find(c => c.id === scene.assignedCharacter);
        if (char) {
           finalPrompt = `A ${char.gender} in ${char.age}, ${char.style}. The person is doing this action: ${finalPrompt}`;
           
           // [NEW] 참조 이미지가 있다면 Base64 로 추출 (퓨전 생성용)
           if (char.referenceImageUrl) {
             // URL 이 data: URI 인지 확인
             if (char.referenceImageUrl.startsWith('data:')) {
               referenceImageBase64 = char.referenceImageUrl.split(',')[1];
             } else {
               // 외부 URL 인 경우 fetch 해서 Base64 로 변환
               try {
      ...
      // [NEW] 참조 이미지와 함께 생성 (캐릭터 일관성 유지)
      const result = await generateImageWithImagen(finalPrompt, '', { aspectRatio: '9:16' }, undefined, referenceImageBase64);
      ...
      const updatedScenes = scenes.map((item) => item.id === sceneId
        ? { ...item, imageUrl: savedImageUrl, isImageGenerating: false, longPrompt: finalPrompt }
        : item
      );
  ...
  const getSelectedDeliveryPrompt = () => {
    ...
    return `${delivery?.prompt || 'Say naturally:'} at ${ttsSpeed}x speed:`;
  };
  ...
  const createGeminiTtsSpeakers = (voiceId: string, text: string) => [
    {
      name: 'Speaker',
      voice: voiceId,
      delivery: getSelectedDeliveryPrompt(),
      dialogue: text
    },
  ...
  const generateGeminiTtsAudioUrl = async (voiceId: string, text: string) => {
    const response = await generateMultiSpeakerAudio(createGeminiTtsSpeakers(voiceId, text));
  ...
  const handlePreviewTts = async () => {
    ...
    try {
      const testText = buildPreviewSampleText('안녕하세요. 이 목소리 샘플입니다.');
      const audioUrl = await generateGeminiTtsAudioUrl(selectedVoice, testText);
      ...
      audio.play().catch(() => {
        // 브라우저 자동재생 제한 시 하단 오디오 플레이어로 수동 재생
      });
      showToast('미리듣기 생성 완료', 'success');
    } catch (err) {
  ...
  const renderContent = () => {
    ...
    const favoriteVoiceIds = new Set(
      favoriteVoices
        .filter((item) => item?.engine === 'gemini' || !item?.engine)
        .map((item) => item.voice)
    );
    const filteredVoices = GEMINI_VOICE_LIBRARY.filter((voice) => {
      const matchesLanguage = voice.language === ttsLanguage;
      const keyword = voiceSearch.trim().toLowerCase();
      const matchesSearch = !keyword || `${voice.name} ${voice.subtitle} ${voice.id}`.toLowerCase().includes(keyword);
      const matchesFilter =
        voiceFilter === 'all' ||
    ...
    const selectedVoiceMeta = GEMINI_VOICE_LIBRARY.find((voice) => voice.id === selectedVoice);
    switch (activeStep) {
      case 'status-board':
        return (
              ...
              <div className="w-[400px] bg-[#111a24] border border-white/5 rounded-[32px] p-8 space-y-6 flex flex-col overflow-y-auto custom-scrollbar">
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 gap-3">
                      {TTS_ENGINES.map((engine) => (
                        <button
                          ...
                          className={`rounded-2xl border p-4 text-left transition-all ${selectedTtsEngine === engine.id ? 'border-lime-500 bg-lime-500/10' : 'border-white/10 bg-[#0a0f16] hover:border-white/20'}`}
                        >
                          <div className="flex items-center justify-between">
                            <div className="font-bold text-white">{engine.label}</div>
                            {selectedTtsEngine === engine.id && <Check size={16} className="text-lime-400" />}
                          </div>
                          <div className="text-[11px] text-slate-500 mt-1">{engine.description}</div>
                          <div className={`text-[10px] font-black mt-3 inline-flex px-2 py-1 rounded-full ${engine.available ? 'bg-lime-500/10 text-lime-400' : 'bg-amber-500/10 text-amber-300'}`}>{engine.credit}</div>
                        </button>
                      ))}
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-[#0a0f16] border border-white/10 rounded-2xl p-3">
                        <div className="text-[10px] text-slate-500 uppercase font-black">생성 완료</div>
                        <div className="mt-2 text-lg font-black text-lime-400">{generatedSceneCount}</div>
                      </div>
                      <div className="bg-[#0a0f16] border border-white/10 rounded-2xl p-3">
                        <div className="text-[10px] text-slate-500 uppercase font-black">전체 씬</div>
                        <div className="mt-2 text-lg font-black text-white">{scenes.length}</div>
                      </div>
                      <div className="bg-[#0a0f16] border border-white/10 rounded-2xl p-3">
                        <div className="text-[10px] text-slate-500 uppercase font-black">현재 Voice</div>
                        <div className="mt-2 text-lg font-black text-white">{selectedVoiceMeta?.name || selectedVoice}</div>
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-500 uppercase font-black mb-2 px-1">언어</div>
                      <div className="flex gap-2">
                        {LANGUAGE_OPTIONS.map((lang) => (
                          <button
                            key={lang.id}
                            onClick={() => setTtsLanguage(lang.id)}
                            className={`px-3 py-2 rounded-xl text-sm font-bold border transition-all ${ttsLanguage === lang.id ? 'bg-lime-500 text-black border-lime-500' : 'bg-[#0a0f16] border-white/10 text-slate-400'}`}
                          >
                      ...
                      <div className="flex gap-2">
                        {TTS_SPEED_PRESETS.map((speed) => (
                          <button
                            ...
                            className={`px-3 py-2 rounded-xl text-sm font-bold border transition-all ${ttsSpeed === speed ? 'bg-lime-500 text-black border-lime-500' : 'bg-[#0a0f16] border-white/10 text-slate-400'}`}
                          >
                            {speed}x
                          </button>
                        ))}
                    ...
                    <div className="bg-[#0a0f16] border border-white/10 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-blue-500/20 text-blue-300 font-black flex items-center justify-center">나</div>
                        <div className="flex-1">
                          <div className="text-sm font-bold text-white">나레이션</div>
                          <div className="text-[11px] text-slate-500">
                            {selectedVoiceMeta ? `${selectedVoiceMeta.name} (${selectedVoiceMeta.id})` : selectedVoice}
                          </div>
                        </div>
                      </div>
                      <input
                        ...
                        placeholder="Gemini 보이스 검색"
                        className="w-full bg-black/30 border border-white/5 rounded-xl px-3 py-2 text-sm text-slate-200 outline-none"
                      />
                      <div className="flex gap-2">
                        {FAVORITE_FILTERS.map((filter) => (
                          <button
                            ...
                            className={`px-3 py-2 rounded-xl text-sm font-bold border transition-all ${voiceFilter === filter.id ? 'bg-lime-500 text-black border-lime-500' : 'bg-black/20 border-white/10 text-slate-400'}`}
                          >
                            {filter.label}
                          </button>
                        ))}
                      ...
                      <div className="max-h-72 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                        {filteredVoices.map((voice) => (
                          <div
                            key={voice.id}
                            className={`rounded-xl border p-3 transition-all ${selectedVoice === voice.id ? 'border-lime-500 bg-lime-500/10' : 'border-white/10 bg-black/20'}`}
                          >
                            <div className="flex items-center justify-between gap-3">
                              <div>
                                <div className="text-sm font-bold text-white">{voice.name}</div>
                                <div className="text-[11px] text-slate-500">{voice.subtitle}</div>
                              </div>
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => setSelectedVoice(voice.id)}
                                  className={`px-3 py-2 rounded-lg text-[11px] font-black ${selectedVoice === voice.id ? 'bg-lime-500 text-black' : 'bg-white/5 text-slate-300'}`}
                                >
                                  {selectedVoice === voice.id ? '선택됨' : '선택'}
                                </button>
                                <button
                                  onClick={() => handlePreviewSpecificVoice(voice.id)}
                                  disabled={isPreviewingVoice === voice.id}
                                  className="px-3 py-2 rounded-lg text-[11px] font-black bg-sky-500/10 text-sky-300 hover:bg-sky-500/20"
                                >
                                  {isPreviewingVoice === voice.id ? '생성중' : '미리듣기'}
                                </button>
                              </div>
                            </div>
                          </div>
                        ...
                        {filteredVoices.length === 0 && (
                          <div className="text-xs text-slate-500 py-6 text-center">조건에 맞는 Gemini 보이스가 없습니다.</div>
                        )}
                      </div>
                    </div>
                    <button 
                    ...
                    <div className="bg-[#0a0f16] border border-white/10 rounded-2xl p-4 space-y-3">
                      <div className="text-[10px] text-slate-500 uppercase font-black">미리듣기 샘플 문구</div>
                      <textarea
                        value={previewText}
                        onChange={(e) => setPreviewText(e.target.value)}
                        className="w-full h-24 bg-black/30 border border-white/5 rounded-xl p-3 text-xs text-slate-300 outline-none resize-none"
                      />
                      {previewAudioUrl ? (
                        <audio src={previewAudioUrl} controls className="w-full h-10" />
                      ) : (
                        <div className="text-[11px] text-slate-500">미리듣기 생성 후 여기서 바로 재생할 수 있습니다.</div>
                      )}
                    </div>
                  </div>
                </div>
                      ...
                      {favoriteVoices.map((f, i) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="text-[10px] text-slate-500">{DELIVERY_OPTIONS.find((option) => option.id === f.delivery)?.label || f.delivery}</div>
                          </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {sceneAudioUrls[i] && (
                          <button
                            ...
                            className="px-3 py-2 rounded-xl bg-sky-500/10 text-sky-400 text-[11px] font-black hover:bg-sky-500/20"
                          >
                            재생
                          </button>
                        )}
          ...
          <div className="p-8 space-y-8 animate-in fade-in duration-500 max-w-[1200px] mx-auto pb-20">
                     ...
                     <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                           <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest flex items-center gap-1">
                              <ImageIcon size={12}/> 참조 이미지 (얼굴/스타일)
                           </label>
                           <div
                             onDrop={(e) => {
                               e.preventDefault();
                               const file = e.dataTransfer?.files?.[0];
                               if (file) handleDropImage(char.id, file);
                             }}
                             onDragOver={(e) => e.preventDefault()}
                             className="aspect-[3/4] bg-black/40 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group hover:border-lime-500/50 transition-colors"
                           >
                              {char.referenceImageUrl ? (
                                 <>
                                    <img src={char.referenceImageUrl} className="w-full h-full object-cover" alt="ref" />
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                       <button className="p-2 bg-white/20 rounded-lg hover:bg-white/40"><Edit2 size={16}/></button>
                                       <button onClick={() => handleUpdateCharacter(char.id, 'referenceImageUrl', null as any)} className="p-2 bg-red-500/80 rounded-lg hover:bg-red-500"><Trash2 size={16}/></button>
                                    </div>
                                 </>
                              ) : (
                                 <label className="flex flex-col items-center gap-2 cursor-pointer w-full h-full justify-center p-4 text-center">
                                    <Camera size={24} className="text-slate-500 group-hover:text-lime-400" />
                                    <span className="text-xs text-slate-500 font-medium">드래그 또는 클릭하여 업로드</span>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="space-y-1">
                              <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest px-1">스타일 묘사 (의상, 헤어 등)</label>
                              <textarea 
                                 value={char.style} 
                                 onChange={e => handleUpdateCharacter(char.id, 'style', e.target.value)}
                                 className="w-full h-32 bg-black/40 border border-white/10 rounded-xl p-3 text-xs text-slate-300 outline-none focus:border-lime-500 resize-none leading-relaxed"
                                 placeholder="예: 긴 생머리, 하얀색 블라우스, 검은색 슬랙스..."
                              />
                           </div>
                           
                           <button className="w-full bg-sky-500/10 border border-sky-500/30 text-sky-400 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-sky-500 hover:text-black transition-all">
                              <Wand2 size={14}/> 이미지 기반 자동 묘사
                           </button>
                        </div>
                     </div>
                  </div>
            ...
            <div className="flex justify-center">
              <button
                ...
                className="bg-white/5 border border-dashed border-white/30 text-slate-300 px-6 py-3 rounded-2xl text-sm font-bold hover:border-white/50 hover:text-white transition-all flex items-center gap-2"
              >
                <Plus size={18} /> 캐릭터 추가
              </button>
            </div>
          ...
          <div className="p-8 space-y-8 animate-in fade-in duration-500 max-w-[1400px] mx-auto overflow-x-hidden">
            <div className="flex justify-between items-end border-b border-white/5 pb-6">
              <div>
                ...
                <p className="text-slate-400 text-sm">씬별로 생성할 이미지를 확인하고, 등장할 고정 캐릭터를 지정하세요.</p>
              </div>
              <div className="flex items-center gap-6">
                <div className="flex flex-col gap-1">
                   <label className="text-[10px] text-slate-500 font-black uppercase px-1">Global Effect</label>
                  ...
                  <div className="p-6 space-y-5 flex-1 flex flex-col bg-[#0a0f16]">
                    ...
                    <div className="space-y-1 flex-1">
                       <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest flex justify-between items-end">
                          생성 프롬프트 
                          <button className="text-sky-400 hover:text-sky-300 text-[10px] font-medium flex items-center gap-1"><Edit2 size={10}/> 편집</button>
                       </label>
                       <div className="w-full h-24 bg-black/40 border border-white/5 rounded-xl p-3 text-[11px] text-slate-400 leading-snug overflow-y-auto custom-scrollbar">
                          {scene.assignedCharacter ? (
                             <div className="text-lime-400/80 mb-1 border-b border-lime-500/20 pb-1">[캐릭터 속성 주입됨] {characters.find(c => c.id === scene.assignedCharacter)?.style}</div>
                          ) : null}
                          {scene.longPrompt || scene.shortPrompt || scene.text}
                       </div>
                    </div>
                  </div>
                </div>
