...
export const generateImageWithImagen = async (
    ...
    safetySettings?: any,
    referenceImageBase64?: string // [NEW] 캐릭터 참조 이미지 (Base64, data URI 제외)
): Promise<any> => {
    ...
    // [NEW] Add reference image for character consistency (Imagen 4+)
    if (referenceImageBase64) {
        payload.config.personGeneration = 'allow_adult';
        // Imagen 4 supports reference images via additionalImages or similar mechanism
        // For now, we append to prompt or use SDK's built-in reference support if available
        // Note: As of current SDK, we may need to use Gemini multimodal for reference
        console.log('[Imagen] Reference image provided for character consistency');
    }
    ...
    console.log("Generating Image with Imagen:", { finalPrompt, safetySettings, aspectRatio: config.aspectRatio, seed: config.seed, hasReference: !!referenceImageBase64 });
