...
export const fetchExtractionCache = async (): Promise<ExtractionCache> => {
  try {
  const response = await fetch(buildApiUrl('/api/extraction-cache'));
...
export const saveExtractionImage = async (imageData: string, type: 'outfit' | 'face' | 'hair' | 'body') => {
  try {
  const response = await fetch(buildApiUrl('/api/extraction-image'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const fetchExtractionImageData = async (filename: string) => {
  try {
  const response = await fetch(`${buildApiUrl('/api/extraction-image')}?filename=${encodeURIComponent(filename)}`);
    if (!response.ok) throw new Error('failed');
...
export const resetExtractionCache = async (): Promise<boolean> => {
  try {
  const response = await fetch(buildApiUrl('/api/extraction-cache/reset'), {
      method: 'POST'
    });
    if (!response.ok) throw new Error('failed');
    return true;
