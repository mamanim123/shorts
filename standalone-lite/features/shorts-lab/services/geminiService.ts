...
type AutomationGenerationPayload = {
  rawResponse?: string;
  text?: string;
  result?: string;
  error?: string;
...
const requestRawGeneration = async (prompt: string, signal?: AbortSignal): Promise<string> => {
  ...
  const payload = (await response.json().catch(() => ({}))) as RawGenerationPayload;

  if (!response.ok) {
    throw new Error(payload.error || 'Failed to generate raw Gemini response');
  }

  const text = payload.rawResponse || payload.text || payload.result || '';
  if (!text.trim()) {
    throw new Error('Empty raw Gemini response');
  }

  return text;
};

const requestAutomationGeneration = async (prompt: string, signal?: AbortSignal): Promise<string> => {
  ...
  const payload = (await response.json().catch(() => ({}))) as AutomationGenerationPayload;
  if (!response.ok) {
    throw new Error(payload.error || 'Failed to generate via automation');
  }
...
// [NEW] TubeFactory Wizard: Generate Trending Storylines
...
export interface BenchmarkStorylinePackage {
  ...
  storylines: { title: string; content: string }[];
}
...
The user wants to benchmark an existing script or content pattern without copying it directly.

[USER TARGET TOPIC]
${normalizedTopic || '자유 주제'}

[BENCHMARK MATERIAL]
${normalizedBenchmark}

Analyze the benchmark material and return a JSON object with:
1. "analysis": concise Korean insights about why the material works
2. "storylines": ${count} fresh Korean storyline ideas that preserve the engagement logic but change the specific topic/content

Return ONLY valid JSON in this exact shape:
{
  "analysis": {
    "sourceSummary": "원본 핵심 요약",
    "hookPattern": "후킹 방식 분석",
    "narrativeStructure": "전개 구조 분석",
    "toneStyle": "톤/말투 분석",
    "narrationHabit": "나레이션 습관 분석",
    "rebuildProtocol": "새 대본에 적용할 재구성 규칙"
  },
  "storylines": [
    {
      "title": "짧고 강한 훅 제목",
      "content": "원본 구조만 참고한 새로운 1-2문장 줄거리"
    }
  ]
}
...
Rules:
- Output 100% Korean.
- Do not copy specific names, lines, or scenes from the benchmark.
- Preserve only the structural strengths: hook, escalation, emotional turn, twist, lesson.
- Make each storyline usable for immediate script generation.`
    : `You are a viral YouTube Shorts/TikTok content planner. The user provided the topic: "${normalizedTopic}".
Generate ${count} highly engaging Korean storyline ideas that grab attention in the first 3 seconds.
Return ONLY valid JSON in this exact shape:
{
  "storylines": [
    {
...
Rules:
- Output 100% Korean.
- Every title must feel clickable and emotional.
- Every content field must be specific enough to use for immediate script generation.
- Do not output analysis, explanation, markdown, or extra text.
- storylines must contain exactly ${count} items.`;

  try {
    const text = await requestGeneration(prompt);
    const parsed = parseJsonFromText<BenchmarkStorylinePackage>(text, [
      'sourceSummary',
      'hookPattern',
      'narrativeStructure',
      'toneStyle',
    ...
    if (parsed && Array.isArray(parsed.storylines) && parsed.storylines.length > 0) {
      return {
        analysis: parsed.analysis || (normalizedBenchmark ? null : {
          sourceSummary: '직접 입력 주제를 기반으로 줄거리 후보를 생성했습니다.',
          hookPattern: '강한 첫 문장 중심',
          narrativeStructure: 'hook -> buildup -> twist -> takeaway',
          toneStyle: '자연스러운 한국어 구어체',
          narrationHabit: '나레이션 중심, 핵심 대사 최소화',
          rebuildProtocol: '선택한 줄거리를 60초 쇼츠형으로 확장'
        }),
    ...
    console.error('Failed to generate benchmark storyline package:', e);
  }

  return {
    analysis: normalizedBenchmark
      ? {
          sourceSummary: '벤치마킹 자료를 기반으로 한 구조 분석을 불러오지 못했습니다.',
          hookPattern: '강한 첫 문장과 감정 유발 요소를 우선 배치합니다.',
          narrativeStructure: 'Hook -> Build -> Twist -> Closing 구조를 유지합니다.',
          toneStyle: '자연스러운 한국어 구어체를 사용합니다.',
          narrationHabit: '나레이션 중심으로 짧고 빠르게 전개합니다.',
          rebuildProtocol: '원본 문장을 복사하지 말고 구조만 유지해 새 소재로 재작성합니다.',
        }
      : null,
    storylines: [
      { title: '시스템 오류', content: 'AI 응답이 지연되었습니다. 잠시 후 다시 시도해주세요.' },
    ],
  };
};

export const generateTrendingStorylines = async (topic: string, count: number = 6): Promise<{title: string, content: string}[]> => {
  const result = await generateBenchmarkStorylinePackage(topic, count);
...
export const generateStory = async (input: UserInput, signal?: AbortSignal, template?: StyleTemplate): Promise<StoryResponse> => {
    ...
    try {
      ...
      const outputScenes = normalizedScenes.map((scene) => {
        ...
        return {
          ...
          soraPromptKo: scene.shortPromptKo || ''
        };
      });
  ...
  try {
    const rawText = await requestGeneration(fullPrompt, signal);
    const parsed = parseJsonFromText<StoryResponse>(rawText, [
      'title',
      'titleOptions',
      'scriptBody',
      'punchline',
    ...
    if (!parsed || typeof parsed !== 'object') {
      throw new Error('Failed to parse story JSON from raw Gemini response.');
    }
...
/**
 * 캐릭터 설명을 AI 최적화된 영문 프롬프트로 변환 (Phase 1: Smart Prompt Bridge)
 */
export const optimizeCharacterPrompt = async (koreanDescription: string): Promise<string> => {
  if (!koreanDescription.trim()) return '';

  const prompt = `
    [TASK]
    Translate and optimize the following Korean character description into a professional AI image generation prompt (Stable Diffusion/Midjourney style).

    [KOREAN DESCRIPTION]
    ${koreanDescription}

    [INSTRUCTIONS]
    1. Translate to descriptive English.
    2. Add specific visual quality keywords (e.g., photorealistic, 8k, cinematic lighting).
    3. Define key attributes clearly: Face features, Hair style, Body type, and Outfit.
    4. Maintain a consistent "Luxury & Glamour" tone.
    5. Output ONLY the optimized English prompt text. No explanations.
  ...
  try {
    ...
    const result = payload.rawResponse || payload.text || payload.result || '';
    return result.trim().replace(/^"|"$/g, '');
  } catch (error) {
    console.error('[geminiService] optimizeCharacterPrompt failed:', error);
    throw error;
  }
};

/**
 * 캐릭터 일관성 유지를 위해 씬 프롬프트에 고정 캐릭터 속성 주입
 */
export const injectCharacterConsistency = (
  scenePrompt: string,
  characters: any[]
): string => {
  ...
  characters.forEach((char, index) => {
    ...
    const charName = char.name || `Character ${String.fromCharCode(65 + index)}`;
    const physicalTraits = char.aiOptimizedPrompt || char.style || '';
    
    if (!physicalTraits) return;

    // 씬 프롬프트에서 캐릭터 이름이 언급된 경우 해당 속성으로 교체/보강
    const nameRegex = new RegExp(`\\b${charName}\\b`, 'gi');
    if (nameRegex.test(enhancedPrompt)) {
      enhancedPrompt = enhancedPrompt.replace(nameRegex, `${charName} (${physicalTraits})`);
    } else if (index === 0 && !enhancedPrompt.includes(physicalTraits)) {
      // 메인 캐릭터 속성이 누락된 경우 맨 앞에 주입
      enhancedPrompt = `${physicalTraits}, ${enhancedPrompt}`;
    }
  });
...
/**
 * 캐릭터 이미지로부터 신체적 특징을 추출하는 Vision AI 브릿지
 * @param imageBase64 
 * @returns 
 */
export const describeCharacterFromImage = async (imageBase64: string) => {
  try {
    ...
    console.error('Error in describeCharacterFromImage:', error);
    throw error;
  }
};
