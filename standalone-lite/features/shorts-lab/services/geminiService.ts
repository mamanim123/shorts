...
Return ONLY valid JSON in this exact shape:
{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || 'Failed to generate benchmark storyline package');
    }
    const payload = await response.json();
    const text = payload?.rawResponse || payload?.text || JSON.stringify(payload);
