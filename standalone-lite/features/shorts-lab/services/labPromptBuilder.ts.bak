...
5. **캔디드 에스테틱 (Candid Aesthetic) & 카메라 시선**:
   - **Scene 1 (Hook)**: 시청자와의 연결을 위해 카메라를 정면 응시하며 미소 지을 것 (Eye contact, smiling at camera).
    - **Scene 2~12**: 자연스러운 일상의 찰나를 포착한 **Candid Shot**이어야 함. 캐릭터는 **카메라를 절대 응시하지 말고(Looking away from camera)** 자신의 행동이나 상대방에게 집중할 것. 인위적인 포즈(Posed) 금지.${golfCaddyRule}
...
⚠️ **미디엄샷 사용 제한**: 전체 씬의 **20% 이하**만 사용! (최대 2개)
...
- scriptBody: 8~12개 문장 (스토리 복잡도와 감정 선에 따라 AI가 자율적으로 결정)
- scenes: 대본의 흐름과 내용에 따라 **최소 8개에서 최대 12개 씬**을 생성
- **TTS 최적화**: 각 scene의 narration.text는 선택된 보이스의 감정적 특징이 잘 드러나도록 생생하게 작성하세요.
- 각 씬은 1개 이상의 scriptBody 문장으로 구성될 수 있으며, 여러 문장을 하나의 씬으로 그룹화할 수 있습니다.
- 예: 복잡한 감정선 → 12문장/12씬, 빠른 템포 → 10문장/8씬 등 융통성 있게 구성
- 각 scene의 scriptLine은 scriptBody의 해당 문장 또는 문장들을 조합하여 일치해야 함
- **Punchline**: 마지막 씬의 대사는 반드시 강렬한 여운이나 반전을 남기는 펀치라인이어야 함.
...
\`\`\`json
{
  ...
  "openingCaption": "첫 1초 자막",
  "openingLine": "첫 문장 Hook",
  "scriptBody": "전체 대본 (${sentenceRange.min}~${sentenceRange.max}문장, 줄바꿈으로 구분)",
  ...
  "scenes": [
    {
      "sceneNumber": 1,
      "scriptLine": "첫 번째 대사 (scriptBody/scriptLines의 첫 번째 문장과 정확히 일치)",
      "characterIds": ["WomanA", "WomanB"],
      "cameraAngle": "drone shot",
      "background": "lush spring golf course with fresh green fairway, warm afternoon sunlight, and soft cherry blossoms swaying near the path",
      "action": "walking side by side across the fairway with light playful smiles, hair moving gently in the spring breeze",
      "emotion": "설렘",
      "summary": "two women appearing on a warm spring golf course with a playful and intriguing mood"
    },
    {
      "sceneNumber": 2,
      "scriptLine": "두 번째 대사",
      ...
      "background": "elegant clubhouse entrance with polished glass, sunlit stone steps, and bright spring trees surrounding the terrace",
      "action": "pausing mid-step with a golf club in hand, glancing back with a subtle smile as the spring light hits her face",
      "emotion": "행복",
      "summary": "a woman drawing attention near the clubhouse in a bright spring atmosphere"
    },
    {
      "sceneNumber": 3,
      "scriptLine": "세 번째 대사",
      ...
      "background": "dramatic fairway lined with tall pine trees and vivid spring greenery under a clear blue sky",
      "action": "striding forward with tense expressions, exchanging quick looks as if something suspicious just happened",
      "emotion": "비장",
      "summary": "two men reacting tensely on the fairway after noticing an awkward moment"
    }
...
2. **titleOptions 필수**: 제목 대안 3개를 작성하세요. 모두 같은 갈등을 가리키되 표현은 다르게.
3. **openingCaption / openingLine 필수**:
   - \`openingCaption\`: 첫 1초 자막용 한 줄
   - \`openingLine\`: 대본의 첫 문장 Hook
   - \`openingLine\`은 반드시 \`scriptLines[0]\` 및 \`scriptBody\` 첫 문장과 동일해야 함
4. **closingPunch / commentTrigger 필수**:
   - \`closingPunch\`: 마지막 반전 대사
...
7. **대본 구조 필수**:
   ...
   - 첫 문장은 스크롤을 멈추게 할 정도로 강해야 하며, 누가 수상한지 / 누가 들켰는지 / 무슨 일이 터졌는지가 즉시 느껴져야 함
   - 첫 문장은 배경 설명으로 시작하지 말 것 ("봄날 골프장에 갔어", "오늘 라운딩을 나갔어" 금지)
   - 첫 문장은 18~32자 내외의 짧고 날카로운 문장으로 작성
   - 아래 3가지 유형 중 하나로만 시작:
     1) 질문형: "여보, 방금 누구한테 한 거야?"
     2) 충격형: "그 윙크 하나에 분위기 끝났어"
     3) 관계 의심형: "남편 친구 쳐다보다가 딱 걸렸어"
   - 전체 문장의 70% 이상을 대사로 작성
   - 구조는 반드시 오해 유발 → 오해 강화 → 반전 폭발
   - 마지막 문장은 가장 강한 반전 대사로 마무리

8. **scriptBody / scriptLines 동시 작성**:
   - \`scriptBody\`: 전체 대본을 줄바꿈 문자열로 작성
   - \`scriptLines\`: 같은 문장을 배열로 다시 작성
   - 두 필드는 문장 수와 순서가 반드시 동일해야 함

9. **씬 구조 필수 항목**:
   - \`sceneNumber\`: 씬 번호 (1부터 시작)
   - \`scriptLine\`: 대사 (scriptBody/scriptLines의 문장과 1:1 대응)
...
10. **배경 일관성 규칙**:
   ...
   - 현재 주제의 계절/장소와 맞지 않는 예시는 절대 섞지 말 것 (예: 봄 주제인데 snow/winter 배경 금지)
...
5. **배경(background) 작성 규칙**:
   ...
   - 좋은 예: "elegant clubhouse lounge with wide glass windows overlooking a bright spring fairway"
   - 좋은 예: "pristine white sand bunker with dramatic shadows"
   - **시각적 요소 포함**: 조명, 분위기, 시간대, 날씨, 특징적 오브젝트
   - **감정 전달**: 배경으로 감정과 분위기를 강화
   - **주제 일치**: 배경의 계절, 날씨, 소품은 반드시 현재 주제와 일치해야 함
...
6. **동작(action) 작성 규칙**:
   ...
   - 좋은 예: "suddenly stopping mid-step and turning back with an awkward smile as everyone watches"
   - **감정과 뉘앙스 포함**: 단순 동작이 아닌 감정이 담긴 행동
   - **세부 묘사**: 손동작, 시선, 표정 변화 등 구체적으로

**체크리스트**:
- [ ] scriptBody는 ${sentenceRange.min}~${sentenceRange.max}개 문장인가?
- [ ] scriptLines는 scriptBody와 문장 수/순서가 동일한가?
- [ ] openingLine = scriptLines 첫 문장인가?
- [ ] closingPunch = scriptLines 마지막 문장인가?
...
export const applyCharacterInfoToScenes = (params: {
  ...
  return scenes.map((scene, sceneIndex) => {
    ...
    if (filteredCharacterBlocks.length === 1) {
      const slotId = filteredCharacterIds[0] || characterIds[0] || '';
      const referenceLabel = slotId ? getReferenceLabel(slotId) : '';
      identityBlock = referenceLabel
        ? `[Reference Character = ${referenceLabel}: ${filteredCharacterBlocks[0]}]`
        : filteredCharacterBlocks[0];
    } else if (filteredCharacterBlocks.length > 1) {
      identityBlock = filteredCharacterBlocks
        .map((block, i) => {
          const slotId = filteredCharacterIds[i] || characterIds[i] || '';
          const referenceLabel = slotId ? getReferenceLabel(slotId) : `Character ${i + 1}`;
          return `[Person ${i + 1} = ${referenceLabel}: ${block}]`;
        })
        .join(' ');
    } else {
